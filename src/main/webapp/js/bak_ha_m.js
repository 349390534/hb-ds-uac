...
(function() {
    ...
    Ajax = function() {
        ...
        oThis.sendByRequest = function(param, callback) {
            ...
            } else if (Request = $HGlobal[_str_window].XMLHttpRequest) {
                Request = new Request;
    ...
    var GaAjax = function(config) {
        ...
        oThis.getHowbuyAnalyticsPath = function() {
            return (window.location.protocol == 'https:') ? "https://ssl.howbuy.com/uac/ha.do" : "http://www.howbuy.com/uac/ha.do";
        }
        ;
        ...
        oThis.send = function(param, account, domainHash, e, d, callback) {
            ...
            if (arr_hutmb[1] < 500 || e) {
                if (d) {
                    ...
                    o = (_time - arr_hutmb[3]) * (_config.Tc / 1E3);
                    if (o >= 1) {
                ...
                if (e || !d || arr_hutmb[2] >= 1) {
                    ...
                    if (0 == localServerMode || 2 == localServerMode) {
                        ...
                        param += "&hutmcch=" + oThis.getRequestParam("'" + location + "'", "hutmcch");
    ...
    var HTracker = function(name, account, namespace) {
        ...
        oThis.getHowbuyAnalyticsUserActionPath = function() {
            return (window.location.protocol == 'https:') ? "https://ssl.howbuy.com/uac/uac.do" : "http://www.howbuy.com/uac/uac.do";
        }
        ;
        oThis.trackHowbuyUserAction = bind("_trackHowbuyUserAction", 100, function(paramJson) {
            ...
            try {
                ...
                var ajax = new Ajax();
                ajax.send(oThis.getHowbuyAnalyticsUserActionPath(), params);
            } catch (err) {}
        });
        oThis.Uc = function() {
            var win = $HGlobal[_str_window];
...
function trackHowbuyUserAction(paramJson) {
    _haq.push(['_trackHowbuyUserAction', paramJson]);
}
...
var statClick = function(e, i) {
	...
	//pageid、pagelevel、tag值都为空的时候不上报
	...
	if (o) {
		...
		//过滤特殊请求  例如“#”号   jvascript:void(0)  以及包含oncilck事件的标签
		if ((n == "a" || n == "area") && (!/^javascript/i.test(a.toLowerCase()) && e.getAttribute("href").substr(0, 1) != "#") && e.getAttribute("id") != "btnAddCart" && o != "71002") {
			if (a.indexOf("HTAG") < 0) {
				...
				if (s < 0 && u > 0 || s > 0 && u > 0 && u < s) {
					...
					//e.href = a.substr(0, u) + "?pageid=" + i.pageid + "&plevel=" + i.plevel + "&tag=" + o + "&" + a.substr(u + 1);
				} else if (s > 0 && (u > s || u < 0)) {
					e.href = a.substr(0, s) + "?HTAG=" + hTag + "&" + a.substr(s + 1);
					//e.href = a.substr(0, s) + "?pageid=" + i.pageid + "&plevel=" + i.plevel + "&tag=" + o + "&" + a.substr(s + 1);
				} else {
					e.href += "?HTAG=" + hTag;
					//e.href += "?pageid=" + i.pageid + "&plevel=" + i.plevel + "&tag=" + o;
				}
			} else {
		...
		} else if (e.getAttribute("href").substr(0, 1) == "#" && e.getAttribute("href").split("#").length > 1) {
...
document.body.onclick=function(o){
	...
	pageId = pageId?pageId:"";
	pagelevel = pagelevel?pagelevel:"";
	statClick(o && o.target, {
		pageID: pageId,
		pagelevel: pagelevel,
		proid: _ha_proid,
		src: "http://www.howbuy.com/uac/hbhc.do"
	});
}
